    ...
    for region in REGIONS_LIST:
        ...
        while True:
            ...
            for park in park_links:
                ...
                parks_list.append((
                    park.get_attribute('href').split("/")[-1],
                    park.text
                    ))
        ...
        parks_list_generator: Generator[tuple[str, str], None, None]
        parks_list_generator = (park for park in parks_list)
        ...
        open(csv_res_path, 'a').close()  # If there is no .csv file, then we create it

        open(csv_res_path, 'w').close()  # Clearing .csv file
        df = pd.DataFrame(city_partners_list,
                          columns=[
                              'PARK_ID',
                              'NAME',
                              'FULL NAME',
